...
cat .id | xargs -I{} grep threads $1/{}/info | cut -d ":" -f 2 | sed 's/[ \t]*//g' > .cores
cat .id | xargs -I{} grep nodes $1/{}/info | cut -d ":" -f 2 |sed 's/[ \t]*//g'  > .nodes
cat .id | xargs -I{} grep MHz $1/{}/info | sed -n '1~16p' | cut -d ":" -f 2 > .freq
cat .id | xargs -I{} grep turbo $1/{}/info | sed -n '1~16p' | cut -d ":" -f 2 > .turbo

paste -d ' ' .id .freq .turbo .nodes .cores > .info
...
sed '1 i id freq turbo nodes cores pkg-clamp-0 pkg-clamp-1 dram-clamp-0 dram-clamp-1 avg-time avg-pkg-0 avg-pp0-0 avg-dram-0 avg-pkg-1 avg-pp0-1 avg-dram-1 tot-pkg-0 tot-pp0-0 tot-dram-0 tot-pkg-1 tot-pp0-1 tot-dram-1' .final-table > table.out

rm -f .id .freq .turbo .cores .nodes .final-table clamp_data row.out node_data.R.*
